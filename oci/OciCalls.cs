...
namespace System.Data.OracleClient.Oci
{
	internal sealed class OciCalls
	{
		...
		internal sealed class OciNativeCalls
		{
...
/*
			internal static void Init () {
				var dllToLoad = "/usr/lib/oracle/12.1/client64/lib/libclntsh.so.12.1";
				var result = dlopen(dllToLoad, 2);
				var errPtr = dlerror();
...
*/
			...
			[DllImport ("clntsh", EntryPoint = "OCIAttrSet")]
			...
			[DllImport ("clntsh", EntryPoint = "OCIPasswordChange")]
			...
			[DllImport ("clntsh", EntryPoint = "OCIBindByName")]
			...
			internal static extern int OCIBindByNameBytes (IntPtr stmtp,
				...
				byte[] valuep,
				int value_sz,
				/*[MarshalAs (UnmanagedType.U2)] OciDataType*/ ushort dty,
				IntPtr indp,
				IntPtr alenp,
				IntPtr rcodep,
				uint maxarr_len,
			...
			[DllImport ("clntsh", EntryPoint = "OCIBindByPos")]
			...
			internal static extern int OCIDefineByPos (IntPtr stmtp,
				...
				IntPtr valuep,
				int value_sz,
				/*[MarshalAs (UnmanagedType.U4)] OciDataType*/ushort dty,
				IntPtr indp,
				IntPtr rlenp,
				IntPtr rcodep,
				uint mode);

			[DllImport ("clntsh", EntryPoint="OCIDefineByPos")]
			...
			[DllImport ("clntsh", EntryPoint = "OCIAttrGet")]
			...
			uint mode)
		{
			...
			return OciNativeCalls.OCIBindByNameBytes (stmtp, out bindpp, errhp, placeholder, placeh_len, valuep,
				value_sz, (ushort)dty, indp, alenp, rcodep, maxarr_len, curelp, mode);
		}
			...
			uint mode)
		{
			...
			return OciNativeCalls.OCIDefineByPos (stmtp, out defnpp, errhp, position, valuep,
				value_sz, (ushort)dty, indp, rlenp, rcodep, mode);
		}
			...
			out int rsize)
		{
			...
			(uint)(dst != null ? dst.Capacity : 0), 
						src, (uint)src.Length, out retSize);
			rsize = (int) retSize;
			return(rc);
		}
		...
		private static bool IsUnix = !System.Runtime.InteropServices.RuntimeInformation.IsOSPlatform(OSPlatform.Windows);
